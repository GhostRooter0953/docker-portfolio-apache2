...
sudo apt update 1>/dev/null 2>&1 && sudo apt install apt-transport-https ca-certificates curl software-properties-common 1>/dev/null 2>&1 && sleep 1.5
...
sudo apt update && apt-cache policy docker-ce | grep "Кандидат:" && sleep 1.5
echo -e "\033[32m - Готово."
echo
echo -e "\033[33m - Устанавливаем docker и проверяем статус его демона."
sudo apt install docker-ce -y 1>/dev/null 2>&1 && echo -e "\033[35m" && sudo  systemctl status docker | grep "Active:" && sleep 1.5
echo -e "\033[32m - Готово."
echo
echo -e "\033[33m - Добавим текущего пользователя в группу docker."
sudo usermod -aG docker ${USER} && sleep 1.5
